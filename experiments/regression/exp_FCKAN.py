...
from src.models.tser.FCNKAN import FCKAN  # agora requer input_channels
...
def evaluate_on_test(model, checkpoint, test_loader, device):
...
for dataset in DATASETS:
    ...
    for exp in range(NUMBER_OF_EXPERIMENTS):
        model = FCKAN(input_channels, rnd_state=42, device=device)  # passa numero de canais
        ...
        out = FCNTrainer(
            ...
            num_epochs=200,
            path2bestmodel=f"weights/fckan_reg_{EXPERIMENT_DATE}",
            dname=datam.dataset_name,
            idx=exp,
            device=device
        )

        with open(f'losses/tser/{dataset}.json', 'w') as f:
            json.dump(out['train_losses'], f)

        # avalia no test_loader
        ckpt = f"weights/fckan_reg_{EXPERIMENT_DATE}/{datam.dataset_name}_run_{exp}_best.pth"
        metrics = evaluate_on_test(model, ckpt, test_loader, device)
        elapsed = time.time() - start

        scatter_ytrue_ypred(metrics['y_true'], metrics['y_pred'],
                            title=f"{dataset}",
                            save_path=f'scatter/{dataset}.png')
        ...
        pd.DataFrame(results_data_dir).to_csv(f"{RESULTS_DIR}FCKAN_REG_{EXPERIMENT_DATE}.csv", index=False)
    print("Done.")