...
from src.models.tser.MLP import MLP
...
for dataset in DATASETS:
    ...
    if first_batch[0].ndim > 2:
        input_channels = first_batch[0].shape[1] * first_batch[0].shape[2]  # C * L
    else:
        input_channels = first_batch[0].shape[1]  # L
    
    for exp in range(NUMBER_OF_EXPERIMENTS):
        mlp = MLP(input_channels)  # passa numero de canais
        ...
        out = MLPTrainer(
            mlp, train_loader, criterion, optimizer,
            num_epochs=100,
            path2bestmodel=f"outputs/weights/mlp_reg_{EXPERIMENT_DATE}",
            dname=datam.dataset_name,
            idx=exp,
            device=device
        )

        with open(f'outputs/losses/tser/{dataset}.json', 'w') as f:
            json.dump(out['train_losses'], f)

        # avalia no test_loader
        ckpt = f"outputs/weights/mlp_reg_{EXPERIMENT_DATE}/{datam.dataset_name}_run_{exp}_best.pth"
        metrics = evaluate_on_test(MLP, ckpt, input_channels, test_loader, device)
        ...
        pd.DataFrame(results_data_dir).to_csv(f"{RESULTS_DIR}MLP_REG_{EXPERIMENT_DATE}.csv", index=False)
    print("Done.")