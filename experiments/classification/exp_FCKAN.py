...
from src.models.tsc.FCKAN import FCKAN
...
def FCNTrainer(model, train_loader, criterion, optimizer,
               num_epochs, path2bestmodel, dname, idx, device, patience=50):
    
    os.makedirs(path2bestmodel, exist_ok=True)
    model.to(device).train()

    checkpoint_best_loss = float('inf')  # para salvar os melhores modelos
    stopping_best_loss = float('inf')    # para early stopping
    train_losses = []
    train_accs = []
    epochs_no_improve = 0
    ...
    for epoch in tqdm(range(1, num_epochs + 1), desc="Epoch"):
        ...
        if avg_loss < checkpoint_best_loss:
            checkpoint_best_loss = avg_loss
            torch.save(model.state_dict(), f"{path2bestmodel}/{dname}_run_{idx}_best.pth")
            melhorou_bool = True
            
        if avg_loss < stopping_best_loss * 0.9:
            stopping_best_loss = avg_loss
            epochs_no_improve = 0
        else:
            epochs_no_improve += 1

        print(f"[{epoch}/{num_epochs}] train-loss: {avg_loss:.4f}  train-acc: {acc:.4f}  patience: {epochs_no_improve}/{patience} melhorou: {melhorou_bool}")

        if epochs_no_improve >= patience:
            print(f"Early stopping at epoch {epoch} (no 10% improvement in train-loss for {patience} epochs)")
            break
...
for dataset in DATASETS:
    ...
    for exp in range(NUMBER_OF_EXPERIMENTS):
        model = FCKAN(n_classes, rnd_state=42, device=device)
        ...
        out = FCNTrainer(model, train_loader, criterion, optimizer,
                         num_epochs=2000,
                         path2bestmodel=f"outputs/weights/tsc_fckan/fckan_tsc_{EXPERIMENT_DATE}",
                         dname=datam.dataset_name,
                         idx=exp,
                         device=device,
                         patience=50)

        with open(f'outputs/losses/tsc/fckan/{dataset}.json', 'w') as f:
            json.dump(out['train_loss'], f)
        
        # 2) avalia só no test_loader
        ckpt = f"outputs/weights/tsc_fckan/fckan_tsc_{EXPERIMENT_DATE}/{datam.dataset_name}_run_{exp}_best.pth"
        ...
        plt.ylabel('True')
        plt.tight_layout()
        plt.savefig(f"outputs/conf_matrix/fckan/{dataset}.png")
        plt.close()
        ...
        pd.DataFrame(results_data_dir).to_csv(f"{RESULTS_DIR}FCKAN_TSC_{EXPERIMENT_DATE}.csv", index=False)
    print("Done.")