...
for dataset in DATASETS:
    ...
    for exp in range(NUMBER_OF_EXPERIMENTS):
        ...
        out = FCNTrainer(fcn, train_loader, criterion, optimizer,
                         num_epochs=2000,
                         path2bestmodel=f"weights/fcn{EXPERIMENT_DATE}",
                         dname=datam.dataset_name,
                         idx=exp,
                         device=device)

        # 2) avalia só no test_loader
        ckpt = f"weights/fcn{EXPERIMENT_DATE}/{datam.dataset_name}_run_{exp}_best.pth"
        ...
        pd.DataFrame(results_data_dir).to_csv(f"{RESULTS_DIR}FCN_{EXPERIMENT_DATE}.csv", index=False)
    print("Done.")